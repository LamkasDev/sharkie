...
func (b *SpvBuilder) EmitImageFetch(resultType, image, coordinate SpirvId, imageOperands ...SpirvId) SpirvId {
	id := b.AllocId()
	operands := append([]uint32{uint32(resultType), uint32(id), uint32(image), uint32(coordinate)})
