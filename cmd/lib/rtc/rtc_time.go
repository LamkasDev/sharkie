...
func libSceRtc_sceRtcGetCurrentClockLocalTime(datetime *RtcDateTime) uintptr {
	if datetime == nil {
	...
	err := SceKernelClockGettime(0, &timestamp)
	if err != 0 {
		return err
	}
	...
	var localTime int64
	var timesec Timesec
	err = SceKernelConvertUtcToLocaltime(epochMicros/1000000, &localTime, &timesec, nil)
	if err != 0 {
		return err
	}
	...
	SceRtcTickAddMinutes(&tick, &tick, adjustment)
	SceRtcSetTick(datetime, &tick)
...
func libSceRtc_sceRtcParseDateTime(tickUtc *RtcTick, datetimeStrPtr Cstring) uintptr {
	if tickUtc == nil || datetimeStrPtr == nil {
	...
	if formatKey == 'Z' || formatKey == '-' || formatKey == '+' || formatKey == '.' {
		return libSceRtc_sceRtcParseRFC3339(tickUtc, datetimeStrPtr)
	} else if formatKey == ':' {
		...
		// Populate result.
		libSceRtc_sceRtcGetTick(&datetime, tickUtc)
		...
		if len(datetimeStr) > 26 && (datetimeStr[26] == '+' || datetimeStr[26] == '-') {
			...
			libSceRtc_sceRtcTickAddMinutes(tickUtc, tickUtc, int64(offset))
		}
	} else {
...
func libSceRtc_sceRtcParseRFC3339(tickUtc *RtcTick, datetimeStrPtr Cstring) uintptr {
	if tickUtc == nil || datetimeStrPtr == nil {
		logger.Printf("%-132s %s failed due to invalid tick or date time string pointer.\n",
			emu.GlobalModuleManager.GetCallSiteText(),
...
func libSceRtc_sceRtcCheckValid(datetime *RtcDateTime) uintptr {
	if datetime == nil {
		logger.Printf("%-132s %s failed due to invalid date time pointer.\n",
			emu.GlobalModuleManager.GetCallSiteText(),
