...
func emitEXP(b *SpvBuilder, instr *Instruction, ctx SpirvBlockContext) {
	switch instr.Details.(*ExpDetails).Target {
	case 0: // target=0 (color)
		val0 := ctx.GetOperandValue(b, instr.Details.(*ExpDetails).VSrcs[0]+OpVgpr0, 0)
		val1 := ctx.GetOperandValue(b, instr.Details.(*ExpDetails).VSrcs[1]+OpVgpr0, 0)
